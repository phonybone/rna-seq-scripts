class Options
  ...
  require 'active_record'         # all for cattr_accessor
  require 'usage_exception'
...
#    set_value(@@opts[name]['alt'],value) unless @@opts[name]['alt']==name
  ...
  def self.parse(*args)
    ...
    args.each do |a|
      ...
      if arg.match(/^-/)
        ...
        unless need_value.nil?
          usage_errors << "missing value for '#{need_value}' (#{arg})" 
          next
        end
        unless known_arg(arg)
          usage_errors << "unknown arg #{arg}" 
          next
        end
        t=arg_type(arg)
        if arg_type(arg)=='b'
    ...
    # check for missing value at end:
      usage_errors << "missing value for '#{need_value}'" unless need_value.nil?

    # check all requried opts are present:
    @@opts.each_pair do |name,a_hash|
        usage_errors << "missing required options '#{name}'" if (a_hash[:required] && a_hash[:value].nil?) 
    end
    if usage_errors.length > 0
      usage_errors.unshift("Usage errors:")
      usage_msg=usage_errors.join("\n")
      raise UsageException, usage_msg
    end
  ...
  def self.use_hash(h)
    raise "use_hash: #{h} not a hash" unless h.class.to_s=='Hash'
    ...
    h.each_pair do |k,v|
      ...
      arg_type=class2chr[v.class]
#      puts "#{k}=#{arg_type} (#{v}, #{v.class})"
      use(k.to_s+'='+arg_type)
      use_defaults({k=>v})
    end
  end
  ...
  def self.default_value(name) default=@@opts[name.to_sym][:default]
    default='<none>' if default.nil? || default=='' 
  end
